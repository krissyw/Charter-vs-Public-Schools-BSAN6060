...
LEFT JOIN LA_schools_2018 s
	ON sta.school_code = s.school_code
WHERE sta.staff_type = "T"
	AND s.charter_school = "No"
GROUP BY sta.school_name
ORDER BY average_age DESC
);

SELECT * FROM avg_teach_age_public_schools_2018;

SELECT * FROM avg_teach_age_charter_schools_2018;
...
FROM LA_schools_2018 s
JOIN num_teachers_charter_2018 num_teach
	ON num_teach.school_code = s.school_code
GROUP BY s.school_name;
